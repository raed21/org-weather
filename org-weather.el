...
(defvar org-weather-format "Weather: %2.2f%tu - ");%desc, %tmin-%tmax%tu, %p%pu, %h%hu, %s%su")

;; Your openweather API key.
(defvar org-weather-api-key  "")

;; The api url, no need to change ususally
(defvar org-weather-api-url "http://api.openweathermap.org/data/2.5/weather?q=%s&mode=json&units=metric&APPID=%s")
...
(defun org-weather-add-to-cache (item)
  ...
  (let* ((timestr (cdr (assoc 'dt item)))
         ...
         ;;(weather (cdr (assoc 'id (assoc 'weather item))))
         ...
         (temperature (number-to-string (cdr (assoc 'temp main))))
         ;;(tmin (number-to-string (cdr (assoc 'temp_min main)))
         ;;(tmax (number-to-string (cdr (assoc 'temp_max main)))
         ;;(feels_like (number-to-string (cdr (assoc 'feels_like main)))
         ;;(presure (number-to-string (cdr (assoc 'presure main)))
         ;;(humidity (number-to-string (cdr (assoc 'humidity main)))
         ...
         (agenda-str (org-weather-replace "%2.2f" temperature agenda-str))
...
(defun org-weather-refresh ()
  ...
  (interactive)
  (setq org-weather-data (make-hash-table :test 'equal))
  (org-weather-add-to-cache (cdr (cdr (cdr (org-weather-load))))) ;;(assoc 'list
